...
# ==============================================================================
# 2. Importul Datelor Reale (EMP_TECH) - VERSIUNEA CSV
# ============================================================================


# Numele fisierului tau CSV
file_emp <- "employment_tech.csv.xlsx"

# Verificam daca exista in folderul data/raw/
if (file.exists(file.path(raw_path, file_emp))) {
  message(paste("Importing EMP_TECH from", file_emp, "..."))

  # Citim CSV-ul.
  # Daca in Excel datele sunt separate prin punct si virgula (;), foloseste read_csv2
  # Daca sunt separate prin virgula (,), foloseste read_csv
  # Vom incerca o citire generica
  df_emp_raw <- read_excel(file.path(raw_path, file_emp), sheet = "Sheet 6", skip = 9, .name_repair = "minimal")

  # Logica de curatare pentru CSV
  # Presupunem ca prima coloana e Tara si ultima coloana e valoarea cea mai recenta

  # Gasim indexul coloanei cu date (cautam 2023 sau 2022)
  cols <- names(df_emp_raw)
  idx_val <- grep("2023", cols)[1] # Cautam 2023
  if (is.na(idx_val)) idx_val <- grep("2022", cols)[1] # Fallback 2022
  if (is.na(idx_val)) idx_val <- ncol(df_emp_raw) # Fallback ultima coloana

  message(paste("Folosim coloana:", cols[idx_val], "pentru datele EMP_TECH"))

  df_emp_clean <- df_emp_raw %>%
    select(1, all_of(idx_val)) %>% # Selectam Geo (Col 1) si Valoarea
    rename(geo_label = 1, EMP_TECH = 2) %>%
    mutate(EMP_TECH = as.numeric(as.character(EMP_TECH))) %>% # Fortam numeric
    filter(!is.na(geo_label))

  # Join cu mapping-ul tarilor (Convertim Nume Tara -> Cod Tara gen RO, DE, FR)
  df_emp_final <- df_emp_clean %>%
    left_join(country_mapping, by = "geo_label") %>%
    filter(!is.na(geo)) %>% # Pastram doar tarile UE mapate corect
    select(geo, EMP_TECH)

  message(">>> SUCCES: EMP_TECH importat si curatat!")
} else {
  message("!!! EROARE: Fisierul employment_tech.csv nu este in folderul data/raw/ !!!")
  message("Se va incerca folosirea datelor simulate (Mock Data), dar analiza nu va fi reala.")
  df_emp_final <- NULL
}
...
for (v in vars_to_check) {
  if (v %in% names(final_df)) {
    if (sum(is.na(final_df[[v]])) > 0) {
      if (all(is.na(final_df[[v]]))) {
        message(paste("Warning: All values missing for", v, "- Using random mock."))
        final_df[[v]] <- runif(nrow(final_df), 10, 50)
      } else {
